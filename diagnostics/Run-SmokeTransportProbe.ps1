param(
    ...
    [string]$OutputRoot="$PSScriptRoot/runs/transport-probe-v1",
    [switch]$Pilot
)
...
$cases=@(foreach($n in @(32,64)){
    [pscustomobject]@{resolution=$n;shape='sharp';shift=0.0;dt_scale=1.0}
    if(-not $Pilot){
        [pscustomobject]@{resolution=$n;shape='sharp';shift=$(if($n -eq 32){0.75}else{0.25});dt_scale=1.0}
        [pscustomobject]@{resolution=$n;shape='sharp';shift=0.0;dt_scale=2.0}
        [pscustomobject]@{resolution=$n;shape='sharp';shift=0.5;dt_scale=0.5}
        [pscustomobject]@{resolution=$n;shape='smooth';shift=0.0;dt_scale=1.0}
    }
})
if($Pilot){$cases=@($cases|Where-Object {$_.resolution -eq 32})}
$jobs=@(foreach($c in $cases){foreach($i in @(-1,[int](512*($c.resolution/32)*($c.resolution/32)),4096,8192)){foreach($probe in @(0,1)){
    ...
    [pscustomobject]@{tag="$($c.shape)-r$($c.resolution)-s$phase-d$dt-i$i-p$probe";case=$c;iterations=$i;probe=$probe}
}}})
$files=@('src/Renderer/SmokeTransportProbe.cpp','src/Renderer/SmokeProjectionExperiment.cpp','src/Renderer/SmokeAdvectionReference.cpp',
    ...
    'diagnostics/Run-SmokeTransportProbe.ps1','diagnostics/analyze_transport_probe.py','diagnostics/analyze_projection.py')
...
@{experiment='transport_probe_v1';utc_started=[DateTime]::UtcNow.ToString('o');git_head=(& git -C $repo rev-parse HEAD);
    executable_sha256=$exeHash;source_sha256=$hashes;cases=$cases;jobs=$jobs;trials=3;steps=2;timing_claim='none; trace copies inside step interval';
...
$names=@('AQUA_SMOKE_REFERENCE','AQUA_SMOKE_REFERENCE_ADVECTION','AQUA_SMOKE_REFERENCE_OUTPUT','AQUA_SMOKE_REFERENCE_OPTIMIZE',
    ...
    'AQUA_SMOKE_PROJECTION_TRIALS','AQUA_SMOKE_PROJECTION_SHIFT','AQUA_SMOKE_PROJECTION_DT_SCALE','AQUA_SMOKE_PROJECTION_STEPS','AQUA_SMOKE_TRANSPORT_PROBE')
...
try{
    ...
    $env:AQUA_SMOKE_PROJECTION_TRIALS='3';$env:AQUA_SMOKE_PROJECTION_STEPS='2'
    foreach($j in $jobs){
        ...
        $env:AQUA_SMOKE_PROJECTION_ITERATIONS="$($j.iterations)";$env:AQUA_SMOKE_TRANSPORT_PROBE="$($j.probe)"
