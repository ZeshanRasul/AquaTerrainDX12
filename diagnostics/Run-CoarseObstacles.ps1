param([string]$OutputRoot="$PSScriptRoot/runs/coarse-obstacles-v1",[string]$Executable="$PSScriptRoot/../out/build/x64-Release/bin/Release/AquaTerrainDX12.exe")
...
$jobs=@(foreach($n in @(32,64)){foreach($mode in @(10,11,12,13)){foreach($cfl in @(0,0.5,2,4)){
    $steps=if($cfl -eq 0){8}else{[int][Math]::Ceiling(0.375*$n/$cfl)}
    [pscustomobject]@{tag="r$n-wall$mode-cfl$cfl";resolution=$n;mode=$mode;cfl=$cfl;steps=$steps}
}}})
...
@{experiment='coarse_obstacle_kinematic_v1';jobs=$jobs;source_sha256=$hashes;executable_sha256=$exeHash;trials=3;utc=[DateTime]::UtcNow.ToString('o');velocity='prescribed, blocked faces zero, not divergence-free';timing_claim='none'} | ConvertTo-Json -Depth 8 | Set-Content (Join-Path $root 'provenance.json')
...
try{
    ...
    foreach($j in $jobs){
        ...
        $env:AQUA_SMOKE_COARSE_OBSTACLE="$($j.mode)";$env:AQUA_SMOKE_OBSTACLE_CFL=$j.cfl.ToString([Globalization.CultureInfo]::InvariantCulture);$env:AQUA_SMOKE_PROJECTION_STEPS="$($j.steps)"
